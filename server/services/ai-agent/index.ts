...
import { recordPromptSnapshot } from './prompt-snapshots';
import { lookupFaqCache } from './faq-cache';
...
export interface AgentResult {
  response: string;
  /** Multiple chunks for humanized split sending. If empty, use `response` as-is. */
  responseChunks?: string[];
  shouldRespond: boolean;
  toolsUsed: string[];
  model: string;
  /** Input tokens consumed by the model (for billing) */
  tokensUsed: number;
  /** Output tokens generated by the model (for billing — Sonnet output is 5× input!) */
...
/** Hard timeout for an Anthropic / OpenAI API call. Prevents workers from hanging. */
const AI_REQUEST_TIMEOUT_MS = 25_000;

/**
 * Creates or retrieves an Anthropic client.
 * Uses company-specific key if available, falls back to global.
 * Includes hard request timeout (25s) and 1 retry to avoid worker starvation.
 */
...
export async function processMessage(
  ...
  // ============================================================
  // ANTI-LOOP LAYER 1: Dedup por messageId (mesma mensagem 2x)
  // ============================================================
  if (options?.wuzapiMessageId) {
    const isDupe = await isDuplicateMessage(companyId, options.wuzapiMessageId);
    if (isDupe) {
      log.info({ messageId: options.wuzapiMessageId }, 'Duplicate messageId, skipping');
      return {
        response: '', shouldRespond: false, toolsUsed: [], model: 'none',
        tokensUsed: 0, outputTokensUsed: 0, processingTimeMs: Date.now() - startTime,
  ...
  // ============================================================
  // ANTI-LOOP LAYER 2: Echo de mensagem outbound (provider devolveu)
  // ============================================================
  // Alguns providers WhatsApp re-entregam ao webhook a mensagem que o bot
  // acabou de enviar. Sem isso, o bot pode entrar em loop infinito.
  if (!options?.fromMe && message) {
    const isEcho = await isOutboundEcho(companyId, normalized, message);
    if (isEcho) {
      log.warn({ companyId, phone: normalized, preview: message.slice(0, 60) },
        'Inbound message matches recent outbound — discarding as echo');
      return {
        response: '', shouldRespond: false, toolsUsed: [], model: 'none',
        tokensUsed: 0, outputTokensUsed: 0, processingTimeMs: Date.now() - startTime,
        isEmergency: false, isHumanTransfer: false, skipResponse: true,
  ...
  // Load conversation state + clinic context (early — needed for takeover timeout)
  const state = await loadConversationState(companyId, sessionId);
  const clinicCtxEarly = await loadClinicContext(companyId);

  // Timeout configurável por clínica via clinic_settings.human_takeover_timeout_minutes.
  // Default 120 min. Override por env var apenas em emergências de operação.
  const humanTakeoverTimeoutMin =
    parseInt(process.env.AI_HUMAN_TAKEOVER_TIMEOUT_MIN || '', 10) ||
    clinicCtxEarly.humanTakeoverTimeoutMinutes ||
    120;

  // ============================================================
  // ANTI-LOOP LAYER 3: fromMe = mensagem ENVIADA pelo atendente humano
  // (ou pelo próprio bot via outro canal). NUNCA responder a essa mensagem.
  // Apenas registrar no histórico e ativar/renovar o modo humano.
  // ============================================================
  if (options?.fromMe) {
    // Persiste a mensagem do atendente como "assistant" para a IA ter contexto
    // quando voltar — assim ela sabe o que o humano já falou.
    try {
      await addAssistantMessage(companyId, sessionId, state, message, {
        toolsUsed: [],
        model: 'human-attendant',
      });
    } catch (err) {
      log.debug({ err }, 'Failed to persist human attendant message');
    }

    state.humanTakeover = true;
    state.humanTakeoverAt = new Date().toISOString();
    state.lastHumanMessageAt = new Date().toISOString();
    state.botShouldResume = false; // resetar — atendente está ativo
    await saveConversationState(companyId, sessionId, state);

    // Update session status
    await db.update(chatSessions).set({ status: 'waiting_human', updatedAt: new Date() })
      .where(eq(chatSessions.id, sessionId));

    log.info({ companyId, phone: normalized, sessionId },
      'Human attendant message recorded — AI in takeover mode');
  ...
  // ============================================================
  // Modo takeover ativo: paciente envia mensagem mas atendente ainda
  // está cuidando. Salva no histórico mas IA permanece silenciosa.
  // ============================================================
  if (isHumanTakeoverActive(state, humanTakeoverTimeoutMin)) {
    ...
    await saveConversationState(companyId, sessionId, state);
    log.info({ companyId, phone: normalized, sessionId },
      'Patient message during human takeover — AI silent');
    return {
      response: '', shouldRespond: false, toolsUsed: [], model: 'none',
      tokensUsed: 0, outputTokensUsed: 0, processingTimeMs: Date.now() - startTime,
      isEmergency: false, isHumanTransfer: false, skipResponse: true,
  ...
  // ============================================================
  // Takeover expirou (atendente parou de responder por X min) —
  // marca para a IA enviar uma saudação de retomada amigável
  // em vez de continuar como se nada tivesse acontecido.
  // ============================================================
  if (state.humanTakeover) {
    state.humanTakeover = false;
    state.humanTakeoverAt = undefined;
    state.botShouldResume = true;
    await db.update(chatSessions).set({ status: 'active', updatedAt: new Date() })
      .where(eq(chatSessions.id, sessionId));
    log.info({ companyId, phone: normalized, sessionId },
      'Human takeover expired — AI will resume with greeting');
  }

  // ============================================================
  // ANTI-LOOP LAYER 4: Throttle de respostas por sessão (max 6/min).
  // Defesa contra qualquer loop não previsto — se acionar, transfere.
  // ============================================================
  const throttle = await shouldThrottleResponse(companyId, sessionId);
  if (throttle.throttle) {
    log.error({ companyId, sessionId, count: throttle.count },
      'Response throttle triggered — transferring to human to break loop');
    const throttleResponse = 'Vou pedir um momento para um atendente assumir a conversa, ok? 🙏';
    await addUserMessage(companyId, sessionId, state, message, options?.wuzapiMessageId);
    await addAssistantMessage(companyId, sessionId, state, throttleResponse);
    state.humanTakeover = true;
    state.humanTakeoverAt = new Date().toISOString();
    await saveConversationState(companyId, sessionId, state);
    await db.update(chatSessions).set({ status: 'waiting_human', updatedAt: new Date() })
      .where(eq(chatSessions.id, sessionId));
    return {
      response: throttleResponse, shouldRespond: true, toolsUsed: [], model: 'throttled',
      tokensUsed: 0, outputTokensUsed: 0, processingTimeMs: Date.now() - startTime,
      isEmergency: false, isHumanTransfer: true, skipResponse: false,
    };
  }

  // OPT-OUT: Detectar solicitação de descadastramento antes de qualquer processamento.
  //
  // CRÍTICO: Match estrito por palavra inteira / frase exata para evitar falsos
  // positivos como "quero CANCELAR minha consulta" (que NÃO é opt-out — é
  // cancelamento de agendamento). Antes usávamos `startsWith('cancelar')`,
  // que derrubava consents indevidamente.
  const messageLower = message.toLowerCase().trim();
  const messageNorm = messageLower.replace(/[.,!?;:]/g, '').replace(/\s+/g, ' ');

  // Frases exatas de opt-out — devem ser a mensagem INTEIRA do paciente
  const OPT_OUT_EXACT = new Set([
    'sair', 'parar', 'pare', 'stop', 'unsubscribe',
    'nao quero mais mensagens', 'não quero mais mensagens',
    'nao quero mais', 'não quero mais',
    'cancelar mensagens', 'cancelar inscricao', 'cancelar inscrição',
    'descadastrar', 'me descadastre', 'me remova',
    'remover meu numero', 'remover meu número',
    'parar de enviar', 'pare de enviar',
  ]);

  const OPT_IN_EXACT = new Set([
    'ativar', 'reativar', 'voltar', 'aceito mensagens',
    'quero receber mensagens', 'pode mandar mensagens',
  ]);

  if (OPT_OUT_EXACT.has(messageNorm)) {
    try {
  ...
  if (OPT_IN_EXACT.has(messageNorm)) {
    try {
      const [patient] = await db.select({ id: patients.id }).from(patients).where(
        and(eq(patients.companyId, companyId), eq(patients.whatsappPhone, normalized))
      ).limit(1);
  ...
  // ============================================================
  // FAQ CACHE: lookup ANTES do Claude para perguntas frequentes
  // ============================================================
  // Hit aqui = 0 tokens, ~5ms latência. Cobra ~25% das perguntas
  // ("qual horário?", "vocês aceitam convênio X?", etc.) sem custo de LLM.
  // Apenas para mensagens curtas (< 200 chars) — perguntas longas
  // raramente são FAQ exata e merecem o LLM.
  if (message && message.length >= 3 && message.length < 200) {
    try {
      const faqHit = await lookupFaqCache(companyId, message);
      if (faqHit) {
        log.info({ companyId, faqId: faqHit.id, preview: message.slice(0, 60) },
          'FAQ cache hit — bypassing LLM');
        await addUserMessage(companyId, sessionId, state, message, options?.wuzapiMessageId);
        await addAssistantMessage(companyId, sessionId, state, faqHit.response, {
          model: 'faq-cache',
        });
        await saveConversationState(companyId, sessionId, state);
        return {
          response: faqHit.response, shouldRespond: true, toolsUsed: [], model: 'faq-cache',
          tokensUsed: 0, outputTokensUsed: 0, processingTimeMs: Date.now() - startTime,
          isEmergency: false, isHumanTransfer: false, skipResponse: false,
        };
      }
    } catch (err) {
      log.debug({ err }, 'FAQ cache lookup failed (non-critical)');
    }
  }
  ...
  // Reuse clinic context loaded earlier (for takeover timeout). Cache TTL = 5min,
  // so this is essentially free even if we re-call loadClinicContext.
  const clinicContext = clinicCtxEarly;
  ...
  // Verifica se há cancelamento pendente NÃO expirado
  let pendingCancelId: number | undefined;
  if (state.pendingCancelAppointmentId && state.pendingCancelExpiresAt) {
    if (new Date(state.pendingCancelExpiresAt).getTime() > Date.now()) {
      pendingCancelId = state.pendingCancelAppointmentId;
    } else {
      // Expirou — limpa
      state.pendingCancelAppointmentId = undefined;
      state.pendingCancelExpiresAt = undefined;
    }
  }

  let systemPrompt = buildSystemPrompt(clinicContext, state.summary, patientContext, {
    resumeFromHumanTakeover: !!state.botShouldResume,
    pendingCancelAppointmentId: pendingCancelId,
  });

  // AUDITORIA: Snapshot do prompt determinístico (sem canary/per-request).
  // Hash em SHA256 + upsert idempotente — apenas variações novas geram INSERT.
  // Este ID será gravado em ai_usage_logs.prompt_snapshot_id permitindo
  // responder "qual prompt a IA usou para clínica X em data Y?".
  let promptSnapshotId: number | null = null;
  try {
    promptSnapshotId = await recordPromptSnapshot(companyId, systemPrompt, {
      botName: clinicContext.botName,
      botPersonality: clinicContext.botPersonality,
      priceDisclosurePolicy: clinicContext.priceDisclosurePolicy,
      humanTakeoverTimeoutMinutes: clinicContext.humanTakeoverTimeoutMinutes,
    });
  } catch (err) {
    log.debug({ err }, 'Prompt snapshot recording failed (non-critical)');
  }

  // Após gerar o prompt com a flag, limpa para não repetir na próxima mensagem
  if (state.botShouldResume) {
    state.botShouldResume = false;
  }
  ...
  try {
    ...
    // CRÍTICO: incluir output_tokens reais — Sonnet output custa 5× input ($15 vs $3 / 1M).
    // Inclui prompt_snapshot_id para auditoria regulatória.
    try {
...
async function runAgentLoop(
  ...
  if (messages.length === 0) {
    return {
      ...
      shouldRespond: true, toolsUsed: [], model,
      tokensUsed: 0, outputTokensUsed: 0,
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    return {
      ...
      shouldRespond: true, toolsUsed, model,
      tokensUsed: inputTokens, outputTokensUsed: outputTokens,
...
export { markOutboundSent, isOutboundEcho } from './conversation-memory';
