...
export function registerProtesesRoutes(app: Express) {
  ...
  // Listar etiquetas de próteses
  app.get("/api/prosthesis/labels",
    authCheck,
    tenantIsolationMiddleware,
    asyncHandler(async (req: Request, res: Response) => {
      const user = req.user as any;
  ...
  // Criar nova etiqueta
  app.post("/api/prosthesis/labels",
    authCheck,
    tenantIsolationMiddleware,
    asyncHandler(async (req: Request, res: Response) => {
      const user = req.user as any;
      ...
      try {
        ...
        const newLabel = await storage.createProsthesisLabel(labelData);
  ...
  // Atualizar etiqueta
  app.patch("/api/prosthesis/labels/:id",
    authCheck,
    tenantIsolationMiddleware,
    asyncHandler(async (req: Request, res: Response) => {
      const user = req.user as any;
      ...
      try {
        const updatedLabel = await storage.updateProsthesisLabel(labelId, req.body, companyId);
        if (!updatedLabel) {
  ...
  app.delete("/api/prosthesis/labels/:id",
    authCheck,
    tenantIsolationMiddleware,
    asyncHandler(async (req: Request, res: Response) => {
      const user = req.user as any;
